...
> Q: Ensure that `countData` and `colData` files match; remove the first
> column in `countData` named `contData$length`.
...
To identify these zero count genes, we can sum across the rows and check
...
Now let’s setup the DESeq Dataset object required for the `DESeq()`
...
this type of data. Basically it’s a plot of log2 fold change vs -log
adjusted p-value.
...
> Q: Let’s reorder these results by adjusted p-value and save them to a
> CSV file in your current project directory.
...
```
    ...
    Info: Working in directory C:/Users/raque/OneDrive/Desktop/BIMM 143/bimm143_github/class14
...
Finally, lets pass these IDs in keggresids to the `pathview()` function
...
![](hsa03030.pathview.png) ![](hsa03013.pathview.png)
